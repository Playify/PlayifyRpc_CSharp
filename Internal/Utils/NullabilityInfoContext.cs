...
namespace System.Reflection{
	...
	public sealed class NullabilityInfoContext{
		...
		internal static bool IsSupported{get;}=
			AppContext.TryGetSwitch("System.Reflection.NullabilityInfoContext.IsSupported",out var isSupported)?isSupported:true;
		...
		private NullabilityState? GetNullableContext(MemberInfo? memberInfo){
			while(memberInfo!=null){
				...
				foreach(var attribute in memberInfo.GetCustomAttributesData()){
		...
		///     If the nullablePublicOnly feature is set for an assembly, like it does in .NET SDK, the private and/or internal
		///     member's
		...
		public NullabilityInfo Create(ParameterInfo parameterInfo){
			...
			var parser=parameterInfo.Member is MethodBase method&&IsPrivateOrInternalMethodAndAnnotationDisabled(method)
				           ?NullableAttributeStateParser.Unknown
				           :CreateParser(attributes);
			var nullability=GetNullabilityInfo(parameterInfo.Member,parameterInfo.ParameterType,parser);
		...
		public NullabilityInfo Create(PropertyInfo propertyInfo){
			...
			var parser=annotationsDisabled?NullableAttributeStateParser.Unknown:CreateParser(propertyInfo.GetCustomAttributesData());
		...
		public NullabilityInfo Create(FieldInfo fieldInfo){
			...
			var parser=IsPrivateOrInternalFieldAndAnnotationDisabled(fieldInfo)?NullableAttributeStateParser.Unknown:CreateParser(attributes);
		...
		private static void EnsureIsSupported(){
			if(!IsSupported){
				throw new InvalidOperationException("NullabilityInfoContext_NotSupported");
			}
		}
		...
		private NullabilityInfo GetNullabilityInfo(MemberInfo memberInfo,Type type,NullableAttributeStateParser parser){
			var index=0;
			var nullability=GetNullabilityInfo(memberInfo,type,parser,ref index);
		...
		private static MemberInfo GetMemberMetadataDefinition(MemberInfo member){
			...
			if(type!=null&&type.IsGenericType&&!type.IsGenericTypeDefinition){
				throw new NotSupportedException("Type.GetMemberWithSameMetadataDefinitionAs doesn't exist");
		...
		private bool TryUpdateGenericParameterNullability(NullabilityInfo nullability,Type genericParameter,Type? reflectedType){
			...
			if(reflectedType is not null
			   &&!(genericParameter.IsGenericParameter&&genericParameter.DeclaringMethod!=null)
		...
		private bool TryUpdateGenericTypeParameterNullabilityFromReflectedType(NullabilityInfo nullability,Type genericParameter,Type context,Type reflectedType){
			Debug.Assert(genericParameter.IsGenericParameter&&!(genericParameter.IsGenericParameter&&genericParameter.DeclaringMethod!=null));
			...
			for(var i=0;i<genericParameter.GenericParameterPosition;i++){
		...
		private bool TryPopulateNullabilityInfo(NullabilityInfo nullability,NullableAttributeStateParser parser,ref int index){
			...
			if(nullability.GenericTypeArguments.Length>0){
				foreach(var genericTypeArgumentNullability in nullability.GenericTypeArguments){
		...
		private static NullabilityState TranslateByte(object? value)=>value is byte b?TranslateByte(b):NullabilityState.Unknown;
